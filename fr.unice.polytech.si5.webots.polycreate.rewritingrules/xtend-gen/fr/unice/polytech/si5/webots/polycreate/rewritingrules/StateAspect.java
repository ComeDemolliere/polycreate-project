...
public class StateAspect {
  @Step
  public static State doActions(final State _self, final PolyCreateControler controler, final EList<Transition> globalTransitions) {
    ...
    Object result = null;
    // #DispatchPointCut_before# State doActions(PolyCreateControler,EList<Transition>)
    if (_self instanceof fr.unice.polytech.si5.webots.polycreate.abstractsyntax.polycreate.State){
    	fr.inria.diverse.k3.al.annotationprocessor.stepmanager.StepCommand command = new fr.inria.diverse.k3.al.annotationprocessor.stepmanager.StepCommand() {
    		@Override
    		public void execute() {
    			addToResult(fr.unice.polytech.si5.webots.polycreate.rewritingrules.StateAspect._privk3_doActions(_self_, (fr.unice.polytech.si5.webots.polycreate.abstractsyntax.polycreate.State)_self,controler,globalTransitions));
    		}
    	};
  ...
  protected static State _privk3_doActions(final StateAspectStateAspectProperties _self_, final State _self, final PolyCreateControler controler, final EList<Transition> globalTransitions) {
    ...
    while ((index < _self.getActions().size())) {
      {
        ...
        for (final Transition t : globalTransitions) {
          ...
          if (_canTransit) {
            ...
            return TransitionAspect.transitToNextState(t, controler, globalTransitions);
          }
        }
        ...
        for (final Transition t_1 : _transitions) {
          ...
          if (_greaterThan) {
            ...
            if (_canTransit_1) {
              ActionAspect.stop(_self.getActions().get(index));
              return TransitionAspect.transitToNextState(t_1, controler, globalTransitions);
            }
          }
        }
      }
