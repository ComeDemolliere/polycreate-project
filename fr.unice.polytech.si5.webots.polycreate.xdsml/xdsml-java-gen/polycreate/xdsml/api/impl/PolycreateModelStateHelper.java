...
public class PolycreateModelStateHelper implements IK3ModelStateHelper{
	...
	public K3ModelState getK3ModelState(EObject model) {
		...
		while (allContentIt.hasNext()) {
			...
			Class<?> clazz =null;
			clazz = K3DslHelper.getTarget(fr.unice.polytech.si5.webots.polycreate.rewritingrules.ActionAspect.class);
			if (clazz.isInstance(elem)) {
				ElementState elemState = theFactory.createElementState();
				elemState.setModelElement(elem);
				res.getOwnedElementstates().add(elemState);
				AttributeNameToValue n2v0 = new AttributeNameToValue("timer", PolycreateRTDAccessor.getTimer((fr.unice.polytech.si5.webots.polycreate.abstractsyntax.polycreate.Action)elem));
				elemState.getSavedRTDs().add(n2v0);
			}
			clazz = K3DslHelper.getTarget(fr.unice.polytech.si5.webots.polycreate.rewritingrules.RobotProgramAspect.class);
			if (clazz.isInstance(elem)) {
				ElementState elemState = theFactory.createElementState();
				elemState.setModelElement(elem);
				res.getOwnedElementstates().add(elemState);
				AttributeNameToValue n2v0 = new AttributeNameToValue("controler", PolycreateRTDAccessor.getControler((fr.unice.polytech.si5.webots.polycreate.abstractsyntax.polycreate.RobotProgram)elem));
				elemState.getSavedRTDs().add(n2v0);
			}
		}
		return res;
	...
	public void restoreModelState(K3ModelState state) {
		for(ElementState elemState : state.getOwnedElementstates()) {
			for(Object o : elemState.getSavedRTDs()) {
				...
				try {
					if (n2v.value != null) {
						Method m = PolycreateRTDAccessor.class.getMethod("set"+n2v.name, EObject.class, n2v.value.getClass());
					...
					for(Class<?> c : n2v.value.getClass().getInterfaces()) {
						...
						try {
							m = PolycreateRTDAccessor.class.getMethod("set"+n2v.name, EObject.class, n2v.value.getClass().getInterfaces()[0]);
							m.invoke(null, elemState.getModelElement(), n2v.value);
